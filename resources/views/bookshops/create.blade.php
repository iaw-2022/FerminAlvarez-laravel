                                ...
                                <tr class="text-center">
                                    ...
                                    <td>{{$book->ISBN}} <br>
                                            <div class="custom-control custom-checkbox">
                                                ...
                                                name="books[]"
                                                value="{{$book->ISBN}}"
                                                onclick="var input = document.getElementById('input{{$book->ISBN}}');
                                                if(this.checked){
                                                    input.disabled = false; input.focus();}
                                                else{input.disabled=true;}">

                                                <input type="number" step="0.1" class="form-control" placeholder="Indique su precio aquí" name="prices[]" id="input{{$book->ISBN}}"  disabled="disabled" required="required">
                                            </div>
                                    </td>
                                    <td>
                                        {{$book->name}}
